...
date: "<br>2016/12/12 <br> A copy of this presentation is available on GitHub: https://github.com/tspeidel/SPE <br> Views are my own."
output:
...
5.   What to Look for in Visualization Tools? An Opinionated List
...
**Data cannot speak for themselves**.  It is in when we interpret data that knowledge is created. My focus is on **bridging the gap between data and knowledge creation**. That gap is filled by analytics and evidence based decision making.
...
```{r, results='markup', fig.align="center", fig.height = 10, fig.width = 10, out.width="500px", out.height="500px", dev="svglite"}
blasingame <- blasingame %>%
     filter(kmax < 100) %>%
     filter(gdens > 0.5) %>%
     as.data.frame() %>%
...
# ggpairs(sample_n(blasingame, 100),
...
library(car)
scatterplot.matrix(~depth + helium.porosity + gdens + k90 + kmax, data=blasingame)
...
```{r, results='markup', fig.height=5.5, fig.width=14}
...
pc <- blasingame %>%
  select(gr, rhob, pe, nphils, lld, lls)

## Use Sparse PC
sparse.pca <- spca(pc, K = 2, type = "predictor", sparse = "varnum", para = c(4, 4))
sparse.pca <- spcabe(pc, nd = 4, thresh = 0.25, unc = FALSE)
data(pitprops)

pc <- spca(x = pc, K = 5, type = "predictor", sparse = "varnum", para = c(5, 5, 5, 5, 5), trace = FALSE)


autoplot(prcomp(pc, scale = TRUE), 
         data = blasingame, 
         colour = blasingame$well,
         loadings = TRUE, 
...
.pull-left[
Smoothers are increadibly useful in order to highlight possible **trends** in the data. One of the most popular and flexible smoother is Cleveland's [**LOESS**](https://en.wikipedia.org/wiki/Local_regression) (locally weighted least squares method).

<small>

* Non-linear
* Built-in outlier control
* Can control degree of smothness
...
.pull-left[
<img src="Figures/cleveland1984.png" width="320px">
]
...
loc.2016 <- loc %>%
  mutate(year = year(time),
         month = month(time)) %>%
  filter(year==2016) %>%
  filter(month %in% c(4, 6, 8)) %>%
  arrange(time) %>%
  dplyr::select(time, lat, lon, activitys, velocity, altitude, heading)

##----------------------------------------------------------------------------------------------------------------------
## VELOCITY
##----------------------------------------------------------------------------------------------------------------------
loc.2016.velocity <- filter(loc.2016, !is.na(velocity))
loc.2016.velocity <- mutate(loc.2016.velocity, velocity.ln = log(velocity))
loc.2016.velocity <- mutate(loc.2016.velocity, velocity.p = as.factor(ntile(velocity, 10)))

pal <- colorFactor(c("RdPu"), loc.2016.velocity$velocity.p)

leaflet(data = loc.2016.velocity) %>%
  setView(lng = -114.062019, lat = 51.044270, zoom = 10) %>%
  addProviderTiles("CartoDB.DarkMatter") %>%
  addProviderTiles("OpenMapSurfer.AdminBounds") %>%
  addCircleMarkers(
    radius = 6,
    color = ~pal(velocity.p),
    stroke = FALSE, fillOpacity = 0.8
  ) %>%
  addLegend(pal = pal, values = ~velocity.p, title = "Velocity (ranks) | Low to High", opacity = 0.9)
...
Remember Tufte: *Numbers become evidence by being in relation to*. Does the tool encourage comparisons? Does it make it easy?

A simple test: I could not create a similar graph in a popular visualization tool:

```{r, results='markup', fig.height=5, fig.width=12}
